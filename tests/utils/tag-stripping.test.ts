...
describe('Tag Stripping Utilities', () => {
  ...
  describe('stripMemoryTagsFromPrompt', () => {
    describe('basic tag removal', () => {
      ...
      it('should strip single <light-mem-context> tag', () => {
        const input = 'public content <light-mem-context>injected context</light-mem-context> more public';
        const result = stripMemoryTagsFromPrompt(input);
        expect(result).toBe('public content  more public');
      });

      it('should strip both tag types in mixed content', () => {
        const input = '<private>secret</private> public <light-mem-context>context</light-mem-context> end';
        const result = stripMemoryTagsFromPrompt(input);
    ...
    describe('multiple tags handling', () => {
      ...
      it('should strip multiple <light-mem-context> blocks', () => {
        const input = '<light-mem-context>ctx1</light-mem-context><light-mem-context>ctx2</light-mem-context> content';
        const result = stripMemoryTagsFromPrompt(input);
      ...
      it('should handle many interleaved tags', () => {
        let input = 'start';
        for (let i = 0; i < 10; i++) {
          input += ` <private>p${i}</private> <light-mem-context>c${i}</light-mem-context>`;
        }
        input += ' end';
        const result = stripMemoryTagsFromPrompt(input);
        expect(result).not.toContain('<private>');
        expect(result).not.toContain('<light-mem-context>');
    ...
    describe('empty and private-only prompts', () => {
      ...
      it('should return empty string for entirely context-tagged prompt', () => {
        const input = '<light-mem-context>all is context</light-mem-context>';
        const result = stripMemoryTagsFromPrompt(input);
        expect(result).toBe('');
      });
      ...
      it('should handle whitespace-only after stripping', () => {
        const input = '<private>content</private>   <light-mem-context>more</light-mem-context>';
        const result = stripMemoryTagsFromPrompt(input);
        expect(result).toBe('');
      });
    });
...
secret
      ...
      it('should strip multiline content within <light-mem-context> tags', () => {
        const input = `start
<light-mem-context>
# Recent Activity
- Item 1
...
finish`;
  ...
  describe('stripMemoryTagsFromJson', () => {
    describe('JSON content stripping', () => {
      ...
      it('should strip light-mem-context tags from JSON', () => {
        const jsonContent = JSON.stringify({
          data: '<light-mem-context>injected</light-mem-context> real data'
        });
        const result = stripMemoryTagsFromJson(jsonContent);
        const parsed = JSON.parse(result);
      ...
      it('should handle tool_response with tags', () => {
        const toolResponse = {
          output: 'result <light-mem-context>context data</light-mem-context>',
          status: 'success'
        };
...
after`;
  ...
  describe('system-reminder tag stripping', () => {
    ...
    it('should strip <system-reminder> mixed with other tag types', () => {
      const input = '<system-reminder>reminder</system-reminder> public <private>secret</private> <light-mem-context>ctx</light-mem-context> end';
      const result = stripMemoryTagsFromPrompt(input);
      expect(result).toBe('public   end');
    });
