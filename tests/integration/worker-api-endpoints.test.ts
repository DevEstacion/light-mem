...
// Mock middleware so tests don't need a real middleware stack.
// In vitest, vi.mock is hoisted to the top of the file automatically.
// Mock both the underlying module and the re-export shim that Server.ts imports.
...
vi.mock('../../src/services/server/Middleware.js', () => ({
  createCorsMiddleware: () => (_req: any, _res: any, next: any) => next(),
  createMiddleware: () => [],
  requireLocalhost: (_req: any, _res: any, next: any) => next(),
  summarizeRequestBody: () => 'test body',
