...
describe('server API key auth', () => {
  ...
  it('creates raw keys once while storing only a salted hash', () => {
    ...
    expect(created.rawKey).toMatch(/^nmem_/);
    ...
    expect(created.record.keyHash).toMatch(/^scrypt\$/);
  ...
  it('verifies a key created with the salted scheme', () => {
    ...
    expect(verifyServerApiKey(db, 'nmem_wrong-key', ['memories:read'])).toBeNull();
  });
  ...
  it('still verifies legacy unsalted SHA-256 keys (#2541 backward compat)', () => {
    ...
    expect(upgraded?.keyHash).toMatch(/^scrypt\$/);
  ...
  it('middleware blocks local-dev bypass when forwarded proxy headers are present', () => {
    ...
    const req: any = {
      ...
      header: (name: string) => {
        ...
        if (normalized === 'host') return 'light-mem.example.com';
  ...
  it('middleware defaults to API-key auth when auth mode is not explicitly set', () => {
    const originalAuthMode = process.env.LIGHT_MEM_AUTH_MODE;
    delete process.env.LIGHT_MEM_AUTH_MODE;
    try {
      ...
      if (originalAuthMode === undefined) {
        ...
        process.env.LIGHT_MEM_AUTH_MODE = originalAuthMode;
      }
    }
  });
