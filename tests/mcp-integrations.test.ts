...
describe('MCP Integrations', () => {
  ...
  describe('writeMcpJsonConfig', () => {
    ...
    it('writes valid JSON with light-mem entry', () => {
      ...
      expect(config.mcpServers['light-mem']).toBeDefined();
      expect(config.mcpServers['light-mem'].command).toBe(process.execPath);
      expect(config.mcpServers['light-mem'].args).toEqual(['/path/to/mcp.cjs']);
    });
    ...
    it('uses custom serversKeyName when provided', () => {
      ...
      expect(config.servers['light-mem']).toBeDefined();
      expect(config.mcpServers).toBeUndefined();
    });

    it('preserves existing servers when adding light-mem', () => {
      const configPath = join(tempDir, 'mcp.json');
  ...
  describe('idempotency', () => {
    it('running install twice does not create duplicate entries', () => {
      ...
      const nordMemEntries = serverKeys.filter((k) => k === 'light-mem');
      expect(nordMemEntries).toHaveLength(1);
    });
    ...
    it('updates the server path on re-install', () => {
      ...
      const config = JSON.parse(readFileSync(configPath, 'utf-8'));
      expect(config.mcpServers['light-mem'].args).toEqual(['/new/path/mcp.cjs']);
    });
  });
