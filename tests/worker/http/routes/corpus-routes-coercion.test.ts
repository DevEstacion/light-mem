...
describe('CorpusRoutes Type Coercion', () => {
  ...
  beforeEach(() => {
    mockBuild = vi.fn((name: string, description: string, filter: any) => Promise.resolve(createCorpus(name, filter)));

    const routes = new CorpusRoutes(
      { list: vi.fn(() => []), read: vi.fn(() => null), delete: vi.fn(() => false) } as any,
