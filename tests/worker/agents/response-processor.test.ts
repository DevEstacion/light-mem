...
describe('ResponseProcessor', () => {
  ...
  beforeEach(() => {
    ...
    mockDbManager = {
      getSessionStore: () => ({
        ...
        ensureMemorySessionIdRegistered: vi.fn(() => {}),  // FK fix (Issue #846)
        getSessionById: vi.fn(() => ({ memory_session_id: 'memory-session-456' })),  // FK fix (Issue #846)
      }),
      getChromaSync: () => ({
    ...
    mockSessionManager = {
      ...
      getPendingMessageStore: () => ({
        markProcessed: vi.fn(() => {}),
        confirmProcessed: vi.fn(() => {}),  // CLAIM-CONFIRM pattern: confirm after successful storage
        cleanupProcessed: vi.fn(() => 0),
        resetStuckMessages: vi.fn(() => 0),
      }),
      confirmClaimedMessages: vi.fn(() => Promise.resolve(0)),
      resetProcessingToPending: vi.fn(() => Promise.resolve(0)),
  ...
  describe('non-XML observer responses', () => {
    it('warns and clears pending work when the observer returns non-XML prose', async () => {
      const confirmClaimedMessages = vi.fn(() => Promise.resolve(0));
      mockSessionManager = {
        getMessageIterator: async function* () { yield* []; },
        getPendingMessageStore: () => ({ confirmProcessed: vi.fn(() => {}) }),
  ...
  describe('parsing summary from XML response', () => {
    ...
    it('should handle response without summary', async () => {
      ...
      (mockDbManager.getSessionStore as any) = () => ({
        ...
        getSessionById: vi.fn(() => ({ memory_session_id: 'memory-session-456' })),
      });
  ...
  describe('error handling', () => {
    it('should reset processing work if memorySessionId is missing from session', async () => {
      const resetProcessingToPending = vi.fn(() => Promise.resolve(1));
      mockSessionManager = {
        getMessageIterator: async function* () { yield* []; },
