...
vi.mock('../../src/services/infrastructure/index.js', () => ({
...
vi.mock('../../src/supervisor/index.js', () => ({
  validateWorkerPidFile: () => 'alive',
}));

vi.mock('../../src/shared/spawn.js', () => ({
...
async function importWorkerUtilsFresh() {
  vi.resetModules();
  return import('../../src/shared/worker-utils.js');
}
...
describe('ensureWorkerRunning — recycle waits for the dying worker\'s successor instead of spawning into the corpse', () => {
  ...
  const originalReadinessBudget = process.env.LIGHT_MEM_HOOK_READINESS_TIMEOUT_MS;
  ...
  beforeEach(() => {
    ...
    // ~/.light-mem lock (a live launcher's lock would make the fallback
    // SKIP its spawn and fail the expectation below).
    tempDataDir = mkdtempSync(join(tmpdir(), 'light-mem-recycle-successor-'));
    process.env.LIGHT_MEM_DATA_DIR = tempDataDir;
    installFetchMock();
    spawnCalls.length = 0;
  });
  ...
  afterEach(() => {
    ...
    if (originalReadinessBudget === undefined) {
      delete process.env.LIGHT_MEM_HOOK_READINESS_TIMEOUT_MS;
    } else {
      process.env.LIGHT_MEM_HOOK_READINESS_TIMEOUT_MS = originalReadinessBudget;
    }
    if (originalDataDir === undefined) {
      delete process.env.LIGHT_MEM_DATA_DIR;
    } else {
      process.env.LIGHT_MEM_DATA_DIR = originalDataDir;
    }
  ...
  it('does NOT spawn when the successor appears on a later health poll', async () => {
    ...
    process.env.LIGHT_MEM_HOOK_READINESS_TIMEOUT_MS = '3000';
  ...
  it('falls back to lazy-spawn when no successor ever appears within the budget', async () => {
    ...
    process.env.LIGHT_MEM_HOOK_READINESS_TIMEOUT_MS = '700';
