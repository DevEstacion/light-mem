...
// settings path at call time via SettingsDefaultsManager.get('LIGHT_MEM_DATA_DIR').
import '../../src/shared/paths.js';

describe('worker-utils API timeout resolution', () => {
  let tempDir: string;
  let settingsPath: string;
  const originalDataDir = process.env.LIGHT_MEM_DATA_DIR;
  const originalTimeout = process.env.LIGHT_MEM_API_TIMEOUT_MS;

  beforeEach(() => {
    tempDir = join(tmpdir(), `worker-timeout-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    mkdirSync(tempDir, { recursive: true });
    settingsPath = join(tempDir, 'settings.json');
    process.env.LIGHT_MEM_DATA_DIR = tempDir;
    delete process.env.LIGHT_MEM_API_TIMEOUT_MS;
    vi.restoreAllMocks();
  });

  afterEach(() => {
    vi.restoreAllMocks();
    if (originalDataDir === undefined) delete process.env.LIGHT_MEM_DATA_DIR;
    else process.env.LIGHT_MEM_DATA_DIR = originalDataDir;
    if (originalTimeout === undefined) delete process.env.LIGHT_MEM_API_TIMEOUT_MS;
    else process.env.LIGHT_MEM_API_TIMEOUT_MS = originalTimeout;
    rmSync(tempDir, { recursive: true, force: true });
  });

  function writeSettings(timeout: string): void {
    const settings = SettingsDefaultsManager.getAllDefaults();
    settings.LIGHT_MEM_DATA_DIR = tempDir;
    settings.LIGHT_MEM_API_TIMEOUT_MS = timeout;
  ...
  it('prefers env timeout over settings.json', async () => {
    writeSettings('45000');
    process.env.LIGHT_MEM_API_TIMEOUT_MS = '1200';
  ...
  it('warns and falls back to default when env timeout is invalid', async () => {
    writeSettings('45000');
    process.env.LIGHT_MEM_API_TIMEOUT_MS = '999999';
    ...
    expect(workerUtils.getWorkerApiRequestTimeoutMs()).toBe(
      parseInt(SettingsDefaultsManager.getAllDefaults().LIGHT_MEM_API_TIMEOUT_MS, 10)
    );
    expect(warnSpy).toHaveBeenCalledWith(
      'SYSTEM',
      'Invalid LIGHT_MEM_API_TIMEOUT_MS, using default',
