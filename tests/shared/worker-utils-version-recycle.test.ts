...
// Must be hoisted so vi.mock factories can reference it safely.
const versionMatchResult = vi.hoisted(() => ({
  value: { matches: true, pluginVersion: '13.4.0', workerVersion: '13.4.0' } as
    { matches: boolean; pluginVersion: string; workerVersion: string | null },
}));
...
vi.mock('../../src/services/infrastructure/index.js', () => ({
  checkVersionMatch: () => Promise.resolve(versionMatchResult.value),
}));
...
function installFetchMock(): void {
  ...
  global.fetch = vi.fn((url: string | URL | Request, init?: RequestInit) => {
    ...
    return Promise.resolve({
      ...
      json: () => Promise.resolve({ version: versionMatchResult.value.pluginVersion }),
...
describe('ensureWorkerRunning — stale-worker recycle on version mismatch', () => {
  ...
  it('POSTs /api/admin/restart when the running worker version differs', async () => {
    versionMatchResult.value = { matches: false, pluginVersion: '13.4.0', workerVersion: '13.3.0' };
  ...
  it('does NOT restart when versions match', async () => {
    versionMatchResult.value = { matches: true, pluginVersion: '13.4.0', workerVersion: '13.4.0' };
