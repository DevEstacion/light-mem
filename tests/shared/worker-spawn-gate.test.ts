...
// which consults LIGHT_MEM_DATA_DIR — so the env var MUST point at the temp
// dir BEFORE the gate module is imported/exercised. Unlike Bun, Vitest/Vite
// does not support runtime query-string cache-busting in dynamic imports.
// However, worker-spawn-gate.ts resolves its lock path at call time via
// resolveDataDir() rather than caching it at module load, so a single static
// import works correctly — each test's beforeEach sets LIGHT_MEM_DATA_DIR to a
// fresh temp dir and the module picks it up on every acquireSpawnLock() call.
import { acquireSpawnLock, releaseSpawnLock } from '../../src/shared/worker-spawn-gate.js';

const ORIGINAL_DATA_DIR = process.env.LIGHT_MEM_DATA_DIR;

async function importGateFresh() {
  return { acquireSpawnLock, releaseSpawnLock };
}
...
describe('worker-spawn-gate — cross-launcher spawn lockfile', () => {
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'light-mem-spawn-gate-'));
  ...
  afterEach(() => {
    if (ORIGINAL_DATA_DIR === undefined) {
      ...
      process.env.LIGHT_MEM_DATA_DIR = ORIGINAL_DATA_DIR;
    }
