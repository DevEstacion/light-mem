 ...
 *
 * Closes anthropics/claude-code#23544 from light-mem's side: installs now
 ...
 * shadow state outside the user's control and competes with light-mem's
...
describe('Install: disable Claude Code auto-memory', () => {
  ...
  describe('disableClaudeAutoMemory runtime behavior', () => {
    ...
    beforeEach(() => {
      tempDir = mkdtempSync(join(tmpdir(), 'light-mem-disable-auto-memory-'));
