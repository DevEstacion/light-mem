...
import type { Database } from '../../src/services/sqlite/node-sqlite-compat.js';

describe('Prompts Module', () => {
  let db: Database;

  beforeEach(() => {
    db = new LightMemDatabase(':memory:').db;
  });
  ...
  describe('saveUserPrompt', () => {
    ...
    it('should store a tag-stripped bounded prompt payload', () => {
      ...
      const oversizedPrompt = `<light-mem-context>ignored</light-mem-context>${'A'.repeat(MAX_STORED_PROMPT_CHARS + 250)}`;
      ...
      expect(stored.prompt_text.startsWith('<light-mem-context>')).toBe(false);
