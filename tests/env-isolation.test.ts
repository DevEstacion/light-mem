 ...
 *
 * Redirect EnvManager to a per-suite temp file via LIGHT_MEM_ENV_FILE so
 * the user's real ~/.light-mem/.env is never read or mutated even if a test
...
const TEST_DATA_DIR = fs.mkdtempSync(join(tmpdir(), 'light-mem-env-isolation-'));
const TEST_ENV_FILE = join(TEST_DATA_DIR, '.env');
const ORIGINAL_ENV_FILE = process.env.LIGHT_MEM_ENV_FILE;
...
describe('Issue #2375: ANTHROPIC_BASE_URL env-var isolation', () => {
  beforeAll(() => {
    ...
    process.env.LIGHT_MEM_ENV_FILE = TEST_ENV_FILE;
  ...
  afterAll(() => {
    ...
    if (ORIGINAL_ENV_FILE === undefined) {
      delete process.env.LIGHT_MEM_ENV_FILE;
    } else {
      process.env.LIGHT_MEM_ENV_FILE = ORIGINAL_ENV_FILE;
    }
  });
  ...
  it('~/.light-mem/.env BASE_URL + AUTH_TOKEN reaches isolatedEnv', () => {
  ...
  it('leaked process.env BASE_URL never reaches the OAuth-skip predicate', async () => {
    ...
    const oauthSpy = vi.spyOn(oauthToken, 'readClaudeOAuthToken');
    try {
