...
/**
 * Regression guard for OpenCode event-contract correctness.
 *
 * The old upstream plugin subscribed to bus event names that do not exist in
 * OpenCode (`session.created`, `message.updated`, `session.compacted`,
 * `file.edited`) and parsed `data.items` instead of the worker's real
 * `data.content` blocks — so it captured nothing and search always returned
 * "No results". These tests fail CI if either contract regresses.
 */
...
describe('OpenCode plugin event contract', () => {
  ...
  it('posts observations to the worker via tool.execute.after', async () => {
    ...
    try {
      const plugin = await LightMemPlugin(pluginCtx);
      const toolAfter = (plugin as Record<string, unknown>)['tool.execute.after'] as (
  ...
  it("parses the worker's real data.content blocks and returns the rows", () => {
    const workerResponse = JSON.stringify({
      content: [
        {
          type: 'text',
          text: 'Found 2 observation(s) matching "auth"\n\n| # | Title |\n|---|---|\n1. Added login flow\n2. Fixed token refresh',
        },
      ],
    });
