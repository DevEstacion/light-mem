...
describe('setup-runtime install marker', () => {
  ...
  describe('writeInstallMarker', () => {
    ...
    it('only writes the documented fields (uv dropped — vector search is in-process)', () => {
  ...
  describe('isInstallCurrent', () => {
    ...
    // isInstallCurrent now compares the resolved Node runtime version (the
    // marker's `bun` field carries it, name kept for format stability).
    const runtimeVersion = process.version.replace(/^v/, '');

    it('returns false when marker version does not match expected', () => {
      mkdirSync(join(tempDir, 'node_modules'));
      writeInstallMarker(tempDir, '1.0.0', runtimeVersion);
      expect(isInstallCurrent(tempDir, '2.0.0')).toBe(false);
    });

    it('returns true when marker matches version and runtime version matches', () => {
      mkdirSync(join(tempDir, 'node_modules'));
      writeInstallMarker(tempDir, '1.0.0', runtimeVersion);
      expect(isInstallCurrent(tempDir, '1.0.0')).toBe(true);
    });

    it('returns false for a matching legacy plain-text marker when a runtime is available', () => {
      mkdirSync(join(tempDir, 'node_modules'));
  ...
  describe('platform remediation strings', () => {
    it('node remediation is non-empty and references Node install', () => {
      const text = platformNodeRemediation();
