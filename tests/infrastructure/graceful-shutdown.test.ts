...
// which must resolve into a temp dir, never the real ~/.light-mem. paths.ts
...
const TEST_DATA_DIR = mkdtempSync(path.join(tmpdir(), 'light-mem-shutdown-test-'));
...
// per-run temp dir (tests/preload.ts), never the real ~/.light-mem. Derive
...
describe('GracefulShutdown', () => {
  ...
  it('resolves the PID file and supervisor registry into a temp dir, never the real ~/.light-mem', () => {
    const realDataDir = path.join(homedir(), '.light-mem');
  ...
  describe('performGracefulShutdown', () => {
    ...
    // REAL ~/.light-mem/supervisor.json before isolation.
