...
// so that file must NEVER be the real ~/.light-mem/worker.pid. paths.ts
...
// and loads nothing.)
const TEST_DATA_DIR = mkdtempSync(path.join(tmpdir(), 'light-mem-pm-test-'));
const PREVIOUS_DATA_DIR = process.env.LIGHT_MEM_DATA_DIR;
process.env.LIGHT_MEM_DATA_DIR = TEST_DATA_DIR;
...
// ~/.light-mem. Derive the paths the assertions use from the SAME frozen
// module the code under test uses, so test and code can never diverge.
const DATA_DIR = paths.dataDir();
const PID_FILE = paths.workerPid();

describe('ProcessManager', () => {
  const REAL_DATA_DIR = path.join(homedir(), '.light-mem');
  ...
  afterAll(() => {
    if (PREVIOUS_DATA_DIR === undefined) {
      ...
      process.env.LIGHT_MEM_DATA_DIR = PREVIOUS_DATA_DIR;
    }
  ...
  describe('test isolation (Phase 6, worker-restart plan)', () => {
    it('resolves the PID file into a temp dir, never the real ~/.light-mem', () => {
  ...
  describe('resolveWorkerRuntimePath', () => {
    // The worker runs under Node now (node:sqlite via the compat adapter), so
    // the resolver simply returns the Node binary that launched the process —
    // no Bun discovery. Hooks invoke us via `node .../node-runner.js`, so
    // process.execPath is already the correct runtime.
    it('returns the provided execPath (the launching Node binary)', () => {
      expect(resolveWorkerRuntimePath({ execPath: '/usr/bin/node' })).toBe('/usr/bin/node');
    });

    it('returns the Windows Node execPath', () => {
      expect(
        resolveWorkerRuntimePath({ execPath: 'C:\\Program Files\\nodejs\\node.exe' })
      ).toBe('C:\\Program Files\\nodejs\\node.exe');
    });

    it('defaults to process.execPath when no override is given', () => {
      expect(resolveWorkerRuntimePath()).toBe(process.execPath);
    });
  });
  ...
  describe('captureProcessStartToken', () => {
    ...
    it.skipIf(!supported)('returns a non-empty token for the current process', () => {
      const token = captureProcessStartToken(process.pid);
      expect(typeof token).toBe('string');
      expect((token ?? '').length).toBeGreaterThan(0);
    });

    it.skipIf(!supported)('returns a stable token across calls for the same PID', () => {
  ...
  describe('writePidFile (start-token capture)', () => {
    ...
    it.skipIf(!supported)('auto-captures a startToken when writing for the current process', () => {
      writePidFile({ pid: process.pid, port: 37777, startedAt: new Date().toISOString() });
  ...
  describe('verifyPidFileOwnership', () => {
    ...
    it.skipIf(!supported)('returns true when the stored token matches the current PID', () => {
      const token = captureProcessStartToken(process.pid);
    ...
    it.skipIf(!supported)('returns false when the stored token does not match (PID reused)', () => {
      expect(verifyPidFileOwnership({
        pid: process.pid,
        port: 37777,
        startedAt: new Date().toISOString(),
  ...
  describe('runOneTimeChromaMigration', () => {
    ...
    beforeEach(() => {
      testDataDir = path.join(tmpdir(), `light-mem-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
