...
describe('error taxonomy', () => {
  it('exposes ErrorSeverity, ERROR_CATEGORIES, classifyError', () => {
    ...
    expect(ERROR_CATEGORIES.length).toBeGreaterThanOrEqual(11);
  });
  ...
  it('classifies a too-old Node error as ABORT (node-runtime-too-old)', () => {
    const cat = classifyError(new Error('Node.js 24.0+ is required (the worker uses the built-in node:sqlite module).'), {
      component: 'node-runtime',
      phase: 'setup-runtime',
    });
    expect(cat.id).toBe('node-runtime-too-old');
...
describe('installerError decision logic', () => {
  ...
  beforeEach(() => {
    ...
    prevDataDir = process.env.LIGHT_MEM_DATA_DIR;
    process.env.LIGHT_MEM_DATA_DIR = home;
  });

  afterEach(() => {
    if (prevDataDir === undefined) delete process.env.LIGHT_MEM_DATA_DIR;
    else process.env.LIGHT_MEM_DATA_DIR = prevDataDir;
  ...
  it('SILENT_RETRY stays silent on first occurrence, escalates on second', () => {
    const summary = createInstallSummary();
    const ctx = { component: 'npm-net', phase: 'setup-runtime', cause: new Error('error: failed to resolve') };
    installerError(ErrorSeverity.SILENT_RETRY, ctx, summary);
    expect(summary.warnings).toHaveLength(0);
    expect(summary.retryCount['npm-net']).toBe(1);
    installerError(ErrorSeverity.SILENT_RETRY, ctx, summary);
    expect(summary.warnings).toHaveLength(1);
    expect(summary.retryCount['npm-net']).toBe(2);
  });
 ...
 */
type Scenario = 'happy' | 'eresolve' | 'old-node';
    ...
    switch (scenario) {
      ...
      case 'old-node':
        installerError(ErrorSeverity.ABORT, {
          component: 'node-runtime',
          phase: 'setup-runtime',
          cause: new Error('Node.js 24.0+ is required (the worker uses the built-in node:sqlite module).'),
        }, summary);
        break;
    }
  } catch (e) {
...
describe('cross-IDE failure matrix (12 IDEs x 4 scenarios)', () => {
  ...
  beforeEach(() => {
    prevMatrixDataDir = process.env.LIGHT_MEM_DATA_DIR;
    process.env.LIGHT_MEM_DATA_DIR = mkdtempSync(join(tmpdir(), 'cm-matrix-'));
  });
  afterEach(() => {
    const dir = process.env.LIGHT_MEM_DATA_DIR;
    ...
    // would expose later test files to the real ~/.light-mem fallback in
    // call-time resolvers.
    if (prevMatrixDataDir === undefined) delete process.env.LIGHT_MEM_DATA_DIR;
  ...
  for (const ide of CANONICAL_IDES) {
    for (const scenario of scenarios) {
      it(`${ide} / ${scenario}`, () => {
        ...
        if (scenario === 'old-node') {
          expect(outcome.remediation).toContain('Node');
        }
