...
import { validateWorkerPidFile, getSupervisor, type ValidateWorkerPidStatus } from '../../src/supervisor/index.js';

function makeTempDir(): string {
  const dir = path.join(tmpdir(), `light-mem-index-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
describe('validateWorkerPidFile', () => {
  ...
  it.skipIf(!tokenSupported)('returns "stale" when startToken does not match the live PID (PID reused)', () => {
    const tempDir = makeTempDir();
    tempDirs.push(tempDir);
    const pidFilePath = path.join(tempDir, 'worker.pid');
    writeFileSync(pidFilePath, JSON.stringify({
