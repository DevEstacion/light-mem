...
// vi.mock is hoisted to top of file — use vi.hoisted() for variables referenced inside factories.
...
vi.mock('../../../src/shared/transcript-parser.js', () => ({
  extractLastMessage: () => {
    mockState.extractCallCount += 1;
    return mockState.mockExtractedMessage;
  },
}));
...
vi.mock('../../../src/shared/worker-utils.js', () => ({
  ...
  workerHttpRequest: (apiPath: string, options?: any) => {
    mockState.workerCallLog.push({ path: apiPath, method: options?.method ?? 'GET', body: options?.body });
  ...
  executeWithWorkerFallback: async (apiPath: string, method: string, body: unknown) => {
    mockState.workerCallLog.push({ path: apiPath, method, body });
...
beforeEach(() => {
  mockState.workerCallLog.length = 0;
  mockState.mockExtractedMessage = '';
  mockState.extractCallCount = 0;
  loggerSpies = [
...
function postedBody(): any {
  expect(mockState.workerCallLog).toHaveLength(1);
  const { body } = mockState.workerCallLog[0];
...
describe('summarizeHandler — privacy tag stripping', () => {
  it('uses Codex lastAssistantMessage directly without reading a transcript', async () => {
    ...
    expect(mockState.extractCallCount).toBe(0);
  ...
  it('short-circuits Codex stop hook re-entry', async () => {
    ...
    expect(mockState.extractCallCount).toBe(0);
    expect(mockState.workerCallLog).toHaveLength(0);
  });

  it('strips <private> tags and their content from last_assistant_message', async () => {
    mockState.mockExtractedMessage ='Hello <private>SECRET-VALUE-42</private> world';
  ...
  it('skips the worker POST when the entire turn is wrapped in a privacy tag', async () => {
    mockState.mockExtractedMessage ='<private>everything is private</private>';
  ...
  it('skips the worker POST when stripping leaves only whitespace', async () => {
    mockState.mockExtractedMessage ='   <private>x</private>\n\t<private>y</private>  ';
  ...
  it('does not modify content that contains no privacy tags', async () => {
    mockState.mockExtractedMessage ='Just a normal assistant turn with no privacy markers.';
  ...
  const taggedPayloads: Array<[string, string]> = [
    ...
    ['<light-mem-context>', '<light-mem-context>SECRET-CTX</light-mem-context>'],
  ...
  for (const [label, payload] of taggedPayloads) {
    it(`strips ${label} tags from last_assistant_message`, async () => {
      ...
      mockState.mockExtractedMessage =`before ${payload} after`;
