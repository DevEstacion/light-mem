...
// near the real ~/.light-mem.
const TEST_DATA_DIR = mkdtempSync(path.join(tmpdir(), 'light-mem-worker-spawn-'));
...
describe('Worker Self-Spawn CLI', () => {
  ...
  describe('status command', () => {
    ...
    // it never reads (or stale-cleans) the real ~/.light-mem/worker.pid.
    it('should report worker status in expected format', async () => {
      const output = runWorkerCommand('status', { LIGHT_MEM_DATA_DIR: TEST_DATA_DIR });
...
describe('Windows-specific behavior', () => {
  ...
  afterEach(() => {
    ...
    delete process.env.LIGHT_MEM_MANAGED;
  });
  ...
  it('should detect Windows managed worker mode correctly', () => {
    ...
    const isManaged = process.env.LIGHT_MEM_MANAGED === 'true';

    expect(isWindows).toBe(true);
    expect(isManaged).toBe(true);

    // Simulate a worker process where IPC (process.send) is not available.
    // In Vitest, process.send may be defined because tests run as IPC workers;
    // temporarily stub it out to test the "no IPC" branch that the original
    // bun test exercised.
