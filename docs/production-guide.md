...
```bash
# Count errors by day
grep '\[ERROR\]' ~/.light-mem/logs/light-mem-*.log | \
  sed 's/\[20[0-9][0-9]-[0-9][0-9]-/\n&/g' | \
  grep -oP '^\[20\d{2}-\d{2}-\d{2}' | sort | uniq -c

# Find circuit-breaker trips
grep 'circuit\|Circuit\|ABANDONED\|abandoned' ~/.light-mem/logs/light-mem-*.log

# Check pending message health
grep 'CLAIMED\|CONFIRMED\|FAILED\|ABANDONED' ~/.light-mem/logs/light-mem-$(date +%Y-%m-%d).log | tail -20
```
