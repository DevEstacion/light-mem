# Progressive Disclosure: Light-Mem's Context Priming Philosophy
...
## Light-Mem's Solution: Progressive Disclosure
...
```markdown
*Use light-mem MCP search to access records with the given ID*
```
...
Light-Mem implements progressive disclosure through a 3-layer workflow pattern:
...
- [Light-Mem Architecture](architecture/overview) - How it all fits together
...
*This philosophy emerged from real-world usage of Light-Mem across hundreds of coding sessions. The pattern works because it aligns with both human cognition and LLM attention mechanics.*
