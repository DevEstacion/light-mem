...
Light-Mem uses an **MCP-based search architecture** that provides intelligent memory retrieval through 4 streamlined tools following a 3-layer workflow pattern.
...
**Location:** `/Users/YOUR_USERNAME/.claude/plugins/marketplaces/light-mem/plugin/scripts/mcp-server.cjs`
...
The 3-layer workflow embodies **progressive disclosure** - a core principle of light-mem's architecture.
...
```json
{
  "mcpServers": {
    "mcp-search": {
      "command": "node",
      "args": [
        "/Users/YOUR_USERNAME/.claude/plugins/marketplaces/light-mem/plugin/scripts/mcp-server.cjs"
      ]
    }
  }
}
...
2. Check database: `ls ~/.light-mem/light-mem.db`
