...
This document describes the PM2 to Bun migration that occurred in v7.1.0 (December 2025). If you're installing light-mem for the first time, this migration has already been completed and you can use the current Bun-based system documented in the main guides.
...
Light-Mem version 7.1.0 introduces two major architectural migrations:
...
- **Logs**: `~/.pm2/logs/light-mem-worker-*.log`
- **PID File**: `~/.pm2/pids/light-mem-worker.pid`

**Lifecycle Commands**:
```bash
pm2 start <script>           # Start worker
pm2 stop light-mem-worker   # Stop worker
pm2 restart light-mem-worker # Restart worker
pm2 delete light-mem-worker  # Remove from PM2
pm2 logs light-mem-worker    # View logs
```
...
- **Port File**: `~/.light-mem/.worker.port` (new)
...
- Database file: `~/.light-mem/light-mem.db` (same location)
...
if (!existsSync(pm2MigratedMarker)) {
  try {
    spawnSync('pm2', ['delete', 'light-mem-worker'], { stdio: 'ignore' });
...
`ensureWorkerRunning()` checks if `~/.light-mem/.worker.pid` exists (it doesn't for first run after update)
</Step>
<Step title="Start Worker Decision">
Worker not running → Call `startWorker()`
</Step>
<Step title="Migration Check">
Check if `~/.light-mem/.pm2-migrated` exists
</Step>
<Step title="PM2 Cleanup">
Execute `pm2 delete light-mem-worker` (errors ignored), create marker file
</Step>
...
**Location**: `~/.light-mem/.pm2-migrated`
...
4. **PM2 cleanup**: `pm2 delete light-mem-worker` (old worker terminated)
5. **Marker creation**: `~/.light-mem/.pm2-migrated` with timestamp
...
| `pm2 stop light-mem-worker` | `npm run worker:stop` | Stop worker |
| `pm2 restart light-mem-worker` | `npm run worker:restart` | Restart worker |
| `pm2 delete light-mem-worker` | `npm run worker:stop` | Remove worker |
| `pm2 logs light-mem-worker` | `npm run worker:logs` | View logs |
| `pm2 describe light-mem-worker` | `npm run worker:status` | Detailed status |
...
**Logs**:
```
Old: ~/.pm2/logs/light-mem-worker-out.log
     ~/.pm2/logs/light-mem-worker-error.log

New: ~/.light-mem/logs/worker-YYYY-MM-DD.log
```

**PID Files**:
```
Old: ~/.pm2/pids/light-mem-worker.pid

New: ~/.light-mem/.worker.pid
```
...
New: ~/.light-mem/.worker.pid
     ~/.light-mem/.worker.port
     ~/.light-mem/.pm2-migrated (all platforms)
```

**Database** (unchanged):
```
Same: ~/.light-mem/light-mem.db
```
...
├────┼────────────────────┼─────────┼─────────┼──────────┤
│ 0  │ light-mem-worker  │ online  │ 0       │ 2d 5h    │
└────┴────────────────────┴─────────┴─────────┴──────────┘
```
...
# Or just remove light-mem logs
rm -f ~/.pm2/logs/light-mem-worker-*.log
rm -f ~/.pm2/pids/light-mem-worker.pid
```
...
```
~/.light-mem/
├── light-mem.db          # Database (unchanged)
...
~/.pm2/
├── logs/
│   ├── light-mem-worker-out.log
│   └── light-mem-worker-error.log
├── pids/
│   └── light-mem-worker.pid
└── pm2.log
```
...
```
~/.light-mem/
├── light-mem.db          # Database (same file)
├── chroma/                # Vector embeddings (unchanged)
├── logs/
...
**Symptoms**:
- `pm2 list` still shows `light-mem-worker`
...
**Manual Resolution**:
```bash
rm ~/.light-mem/.worker.pid
rm ~/.light-mem/.worker.port
...
| `Invalid port X` | Port validation failed | Update `LIGHT_MEM_WORKER_PORT` in settings |
...
# 2. Start PM2 worker
pm2 start plugin/scripts/worker-cli.js --name light-mem-worker
...
# 5. Verify migration
pm2 list  # Should NOT show light-mem-worker
cat ~/.light-mem/.pm2-migrated  # Should exist
