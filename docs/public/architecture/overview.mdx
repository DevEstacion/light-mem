...
description: "System components and data flow in Light-Mem"
---
...
Light-Mem operates as a Claude Code plugin with the following core components:
...
2. **Worker Service** - Express HTTP API on a per-user port; processes observations via the Claude Agent SDK
...
│    "run: npx light-mem repair" to stderr. Always exits 0.      │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
...
The actual runtime install (Bun, uv, `bun install`) is performed by `npx light-mem install` / `npx light-mem repair` with a visible installer spinner; the Setup hook itself only reads the `.install-version` marker.

See [Plugin Hooks](/architecture/hooks) for detailed hook documentation.

### 2. Worker Service
Express.js HTTP server on a per-user port (default `37700 + (uid % 100)`, override via `LIGHT_MEM_WORKER_PORT`) with:
- Search HTTP API endpoints
- Viewer UI HTTP/SSE endpoints
- Async observation processing via the Claude Agent SDK
...
- Location: `~/.light-mem/light-mem.db`
