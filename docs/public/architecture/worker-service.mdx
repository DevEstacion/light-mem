...
- **Port**: Per-user default `37700 + (uid % 100)` (override with `LIGHT_MEM_WORKER_PORT`). The active port is stored in `~/.light-mem/settings.json` and reported by `GET /api/health`.
- **Location**: `src/services/worker-service.ts`
- **Built Output**: `plugin/scripts/worker-service.cjs`
- **Model**: Configurable via `LIGHT_MEM_MODEL` (default: `claude-haiku-4-5-20251001`)
...
The `port` value is the actual worker port for the current user — per-user default `37700 + (uid % 100)`, or whatever `LIGHT_MEM_WORKER_PORT` is set to. The example above is illustrative; your value will differ.
...
- PID file tracking at `~/.light-mem/worker.pid`
...
Bun is required to run the worker service. If Bun is not installed, `npx light-mem install` (and `npx light-mem repair`) installs it globally during setup, with a visible clack spinner:
...
Set the Claude model used for compression via environment variable or `~/.light-mem/settings.json`:

```bash
export LIGHT_MEM_MODEL=claude-haiku-4-5-20251001
```
...
- **Override**: Set `LIGHT_MEM_WORKER_PORT` (env or `~/.light-mem/settings.json`)
- **Discovery**: `GET /api/health` returns the active port; `~/.light-mem/settings.json` stores the configured value

If the chosen port is occupied, the worker fails to start — pin a different port via `LIGHT_MEM_WORKER_PORT` and restart.
...
```
~/.light-mem/
├── light-mem.db           # SQLite database (bun:sqlite)
