---
title: "Configuration"
description: "Environment variables and settings for Light-Mem"
---
...
Settings are managed in `~/.light-mem/settings.json`. The file is auto-created with defaults on first run.

### Core Settings

| Setting                       | Default                         | Description                           |
|-------------------------------|---------------------------------|---------------------------------------|
| `LIGHT_MEM_MODEL`            | `claude-haiku-4-5-20251001`     | Claude model used to compress observations (when using the Claude provider) |
| `LIGHT_MEM_PROVIDER`         | `claude`                        | AI provider: `claude` |
| `LIGHT_MEM_CLAUDE_AUTH_METHOD` | `subscription`                | Claude provider auth mode: `subscription`, `api-key`, or `gateway` |
| `LIGHT_MEM_MODE`             | `code`                          | Active mode profile (e.g., `code--es`, `email-investigation`) |
| `LIGHT_MEM_CONTEXT_OBSERVATIONS` | `50`                        | Number of observations to inject      |
| `LIGHT_MEM_WORKER_PORT`      | `37700 + (uid % 100)`           | Worker service port (per-user default; override for fixed port) |
| `LIGHT_MEM_WORKER_HOST`      | `127.0.0.1`                     | Worker service host address           |
| `LIGHT_MEM_DATA_DIR`         | `~/.light-mem`                 | Data root — every other path (database, chroma, logs, settings.json, worker.pid) derives from this |
| `LIGHT_MEM_SKIP_TOOLS`       | `ListMcpResourcesTool,SlashCommand,Skill,TodoWrite,AskUserQuestion` | Comma-separated tools to exclude from observations |

### Claude Gateway Settings

Gateway credentials live in `~/.light-mem/.env`, not `settings.json`.
...
Use [LiteLLM Gateway](configuration/litellm-gateway) when you want `LIGHT_MEM_PROVIDER=claude` to route through LiteLLM while preserving the Claude Agent SDK worker path.

### System Configuration

| Setting                       | Default                         | Description                           |
|-------------------------------|---------------------------------|---------------------------------------|
| `LIGHT_MEM_DATA_DIR`         | `~/.light-mem`                 | Data directory location               |
| `LIGHT_MEM_LOG_LEVEL`        | `INFO`                          | Log verbosity (DEBUG, INFO, WARN, ERROR, SILENT) |
| `LIGHT_MEM_PYTHON_VERSION`   | `3.13`                          | Python version for chroma-mcp         |
| `CLAUDE_CODE_PATH`            | _(auto-detect)_                 | Path to Claude Code CLI (for Windows) |

## Model Configuration

Configure which Claude model compresses your observations (only applies when `LIGHT_MEM_PROVIDER=claude`).
...
`npx light-mem install` prompts for the Claude model (when the Claude provider is selected) and persists the choice to `~/.light-mem/settings.json`.
...
| Setting | Default | Description |
|---------|---------|-------------|
| `LIGHT_MEM_MODE` | `code` | Defines behavior and language. See [Modes & Languages](modes). |
...
```json
{
  "LIGHT_MEM_MODE": "email-investigation"
}
```
...
- **Production (installed plugin)**: `~/.light-mem/` (always, regardless of CLAUDE_PLUGIN_ROOT)
- **Development**: Can be overridden with `LIGHT_MEM_DATA_DIR`

```
~/.light-mem/
├── light-mem.db           # SQLite database
├── .install-version        # Version marker written by `npx light-mem install`/`repair`
...
Light-Mem provides MCP search tools for querying your project history.
...
- **HTTP API**: 10 endpoints on the worker service port (per-user, default `37700 + (uid % 100)`; see `~/.light-mem/settings.json`)
- **Auto-Invocation**: Claude recognizes natural language queries about past work

## Version Channel

Light-Mem supports switching between stable and beta versions via the web viewer UI.

### Accessing Version Channel

1. Open the viewer at the worker URL (default `http://127.0.0.1:<worker-port>`; the active port is the value of `LIGHT_MEM_WORKER_PORT` in `~/.light-mem/settings.json`)
...
Light-Mem can automatically generate `CLAUDE.md` files in your project folders with activity timelines. This feature is disabled by default.

| Setting | Default | Description |
|---------|---------|-------------|
| `LIGHT_MEM_FOLDER_CLAUDEMD_ENABLED` | `false` | Enable auto-generation of folder CLAUDE.md files |
...
Light-Mem injects past observations into each new session, giving Claude awareness of recent work. You can configure exactly what gets injected using the **Context Settings Modal**.
...
| **Worker Port** | `37700 + (uid % 100)` | Port for background worker service (override with `LIGHT_MEM_WORKER_PORT`) |
...
Settings are stored in `~/.light-mem/settings.json`:

```json
{
  "LIGHT_MEM_CONTEXT_OBSERVATIONS": "100",
  "LIGHT_MEM_CONTEXT_SESSION_COUNT": "20",
  "LIGHT_MEM_CONTEXT_OBSERVATION_TYPES": "bugfix,decision,discovery",
  "LIGHT_MEM_CONTEXT_OBSERVATION_CONCEPTS": "how-it-works,gotcha",
  "LIGHT_MEM_CONTEXT_FULL_COUNT": "10",
  "LIGHT_MEM_CONTEXT_FULL_FIELD": "narrative",
  "LIGHT_MEM_CONTEXT_SHOW_READ_TOKENS": "true",
  "LIGHT_MEM_CONTEXT_SHOW_WORK_TOKENS": "true",
  "LIGHT_MEM_CONTEXT_SHOW_SAVINGS_AMOUNT": "true",
  "LIGHT_MEM_CONTEXT_SHOW_LAST_SUMMARY": "false",
...
Settings can be customized in `~/.light-mem/settings.json`.
...
```json
{
  "LIGHT_MEM_DATA_DIR": "/custom/path"
}
```
...
```json
{
  "LIGHT_MEM_MODEL": "opus"
}
```

Then restart the worker:
```bash
export LIGHT_MEM_MODEL=opus
npm run worker:restart
```

### Custom Skip Tools

Control which tools are excluded from observations. Edit `~/.light-mem/settings.json`:
```json
{
  "LIGHT_MEM_SKIP_TOOLS": "ListMcpResourcesTool,SlashCommand,Skill"
}
```
...
The Setup hook never installs anything — runtime install (Bun, uv, `bun install`) happens in `npx light-mem install` / `npx light-mem repair` outside the session lifecycle.
...
2. Verify environment variables:
   ```bash
   echo $LIGHT_MEM_MODEL
   echo $LIGHT_MEM_WORKER_PORT
   ```
...
If you specify an invalid Claude model name, the worker logs a warning and uses the default. Valid Claude models for `LIGHT_MEM_MODEL`:
...
1. Set custom port:
   ```bash
   export LIGHT_MEM_WORKER_PORT=38000
   ```
...
3. Verify new port:
   ```bash
   curl -s http://127.0.0.1:$LIGHT_MEM_WORKER_PORT/api/health | jq .port
   ```
