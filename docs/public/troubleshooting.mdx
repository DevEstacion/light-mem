...
description: "Common issues and solutions for Light-Mem"
---
...
1. Find the worker port. The default is `37700 + (uid % 100)`. The configured port is the value of `LIGHT_MEM_WORKER_PORT` in `~/.light-mem/settings.json`; the running worker also reports it on `/api/health`:
   ```bash
   PORT=$(jq -r .LIGHT_MEM_WORKER_PORT ~/.light-mem/settings.json)
   lsof -i :$PORT
...
1. Check browser localStorage is enabled:
   ```javascript
   // In browser console
   localStorage.getItem('light-mem-settings')
   ```
...
3. Clear localStorage and try again:
   ```javascript
   // In browser console
   localStorage.removeItem('light-mem-settings')
   ```
...
1. Clear install cache:
   ```bash
   rm ~/.claude/plugins/marketplaces/light-mem/.install-version
   ```
...
3. Check version marker:
   ```bash
   cat ~/.claude/plugins/marketplaces/light-mem/.install-version
   cat ~/.claude/plugins/marketplaces/light-mem/package.json | grep version
   ```
...
3. Check database for corruption:
   ```bash
   sqlite3 ~/.light-mem/light-mem.db "PRAGMA integrity_check;"
   ```
...
3. Verify database has observations:
   ```bash
   sqlite3 ~/.light-mem/light-mem.db "SELECT COUNT(*) FROM observations;"
   ```
...
1. Check if summaries exist:
   ```bash
   sqlite3 ~/.light-mem/light-mem.db "SELECT COUNT(*) FROM session_summaries;"
   ```
...
3. Check database size (large database = slow queries):
   ```bash
   ls -lh ~/.light-mem/light-mem.db
   ```

4. Optimize database:
   ```bash
   sqlite3 ~/.light-mem/light-mem.db "VACUUM;"
   ```
...
3. Check package.json exists:
   ```bash
   ls -la ~/.claude/plugins/marketplaces/light-mem/package.json
   ```
...
2. Backup database:
   ```bash
   cp ~/.light-mem/light-mem.db ~/.light-mem/light-mem.db.backup
   ```
...
1. Check FTS5 tables exist:
   ```bash
   sqlite3 ~/.light-mem/light-mem.db "SELECT name FROM sqlite_master WHERE type='table' AND name LIKE '%_fts';"
   ```
...
3. Check triggers exist:
   ```bash
   sqlite3 ~/.light-mem/light-mem.db "SELECT name FROM sqlite_master WHERE type='trigger';"
   ```
...
2. Verify FTS5 tables populated:
   ```bash
   sqlite3 ~/.light-mem/light-mem.db "SELECT COUNT(*) FROM observations_fts;"
   ```
...
**Symptoms**: `/plugin install light-mem` fails.
...
3. Verify installation:
   ```bash
   ls -la ~/.claude/plugins/marketplaces/light-mem/
   ```
...
jq -r .LIGHT_MEM_WORKER_PORT ~/.light-mem/settings.json
...
**Solution**: Check `~/.light-mem/` exists and is writable.
...
2. **Create issue**: [GitHub Issues](https://github.com/Ronald-Estacion_NordTech/light-mem/issues)
...
- [Configuration](configuration) - Customize Light-Mem
