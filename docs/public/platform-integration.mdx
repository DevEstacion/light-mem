...
description: Complete reference for integrating light-mem worker service into VSCode extensions, IDE plugins, and CLI tools
icon: plug
---

<Note>
**Target Audience:** Developers building light-mem integrations (VSCode extensions, IDE plugins, CLI tools).
</Note>
...
// or whatever the user set via LIGHT_MEM_WORKER_PORT / settings.json. Read it from
// process.env.LIGHT_MEM_WORKER_PORT, then ~/.light-mem/settings.json
// (LIGHT_MEM_WORKER_PORT key), then fall back to the deterministic default.
...
```bash
LIGHT_MEM_MODEL=claude-haiku-4-5-20251001  # Default Claude model for observations/summaries
LIGHT_MEM_CONTEXT_OBSERVATIONS=50          # Observations injected at SessionStart
LIGHT_MEM_WORKER_PORT=                     # Optional override; default = 37700 + (uid % 100)
LIGHT_MEM_DATA_DIR=                        # Optional override for the data directory
...
**Tag Stripping:** Removes `<private>` and `<light-mem-context>` tags before storage.
...
```json Response
{
  "projects": ["light-mem", "other-project", ...]
}
```
...
// Register tool
vscode.lm.registerTool('light-mem-search', searchTool);
```

#### Chat Participant Implementation

```typescript
const participant = vscode.chat.createChatParticipant('light-mem', async (request, context, stream, token) => {
...
class WorkerUnavailableError extends Error {
  constructor() {
    super('Light-Mem worker is not running or unreachable');
...
```json
{
  "name": "light-mem-vscode",
  "displayName": "Light-Mem",
  "version": "1.0.0",
  "engines": {
    "vscode": "^1.95.0"
  },
...
describe('Worker Integration', () => {
  ...
  beforeAll(async () => {
    ...
    workerProcess = spawn('node', ['dist/worker-service.js'], {
      env: { ...process.env, LIGHT_MEM_WORKER_PORT: '37778' }
    });
...
export function registerSearchTool(context: vscode.ExtensionContext) {
  ...
  const searchTool = vscode.lm.registerTool('light-mem-search', {
    description: 'Search persistent memory for observations, sessions, and prompts',
    inputSchema: {
      type: 'object',
      properties: {
...
<CardGroup cols={2}>
  <Card title="Documentation" icon="book" href="https://github.com/Ronald-Estacion_NordTech/light-mem">
    Complete light-mem documentation
  </Card>
  <Card title="GitHub" icon="github" href="https://github.com/Ronald-Estacion_NordTech/light-mem">
