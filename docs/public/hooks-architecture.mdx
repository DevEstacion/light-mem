# How Light-Mem Uses Hooks: A Lifecycle-Driven Architecture
...
Light-Mem is fundamentally a **hook-driven system**. Every piece of functionality happens in response to lifecycle events:
...
**Key insight:** Light-Mem doesn't interrupt or modify Claude Code's behavior. It observes from the outside and provides value through lifecycle hooks.
...
Light-Mem had several architectural constraints:
...
Light-Mem uses lifecycle hook scripts across 5 lifecycle events. Runtime setup is handled out-of-band by `npx light-mem install` (and `npx light-mem repair`); the Setup hook only runs a sub-100ms `version-check.js` to flag stale installs. SessionStart runs 2 hooks in sequence: worker-service start, then context-hook.

### Setup Hook: Version Check

**Purpose:** Detect stale installs caused by external plugin upgrades and prompt the user to repair.

**Note:** Runtime installation (Bun, uv, `bun install` in the plugin cache) is performed by `npx light-mem install` and `npx light-mem repair` — the Setup hook itself never installs anything.
...
3. On mismatch, writes `run: npx light-mem repair` to stderr.
...
**Source:** `scripts/version-check.js`. The matching installer logic lives in `npx light-mem install` / `npx light-mem repair`, which install Bun + uv globally, run `bun install` in the plugin cache, and write the `.install-version` marker — all behind a visible clack spinner.
...
- ✅ Configurable observation count via `LIGHT_MEM_CONTEXT_OBSERVATIONS`

**Output format:**
```markdown
# [light-mem] recent context
...
**Technology:** Express.js REST API on the worker's per-user port (default `37700 + (uid % 100)`, override via `LIGHT_MEM_WORKER_PORT`)
...
<AccordionGroup>
  ...
  <Accordion title="Observations not captured">
    ...
    **Debugging:**
    1. Check database: `sqlite3 ~/.light-mem/light-mem.db "SELECT * FROM observation_queue"`
...
- All heavy lifting (Bun + uv install, `bun install` inside the plugin cache) happens in `npx light-mem install` / `npx light-mem repair`, which run with a visible clack spinner outside the session lifecycle.
- On marker mismatch the hook prints a one-line `run: npx light-mem repair` hint to stderr and exits 0; the user opts into the slow path explicitly.
...
**Privacy guarantees:**
- All data stored locally in `~/.light-mem/light-mem.db`
...
*The hook-driven architecture enables Light-Mem to be both powerful and invisible. Users never notice the memory system working - it just makes Claude smarter over time.*
