...
description: "How light-mem intercepts file reads to save tokens using observation history"
---
...
Projects matching patterns in `LIGHT_MEM_EXCLUDED_PROJECTS` skip the gate entirely. Configure this in `~/.light-mem/settings.json`:

```json
{
  "LIGHT_MEM_EXCLUDED_PROJECTS": "/tmp/*,/scratch/*"
}
```
...
2. Find the light-mem hooks section under `hooks.PreToolUse` and remove the entry with the `Read` matcher.
...
The File Read Gate is one piece of light-mem's layered context strategy:
