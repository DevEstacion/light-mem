...
description: "How light-mem evolved from v3 to v5+"
---
...
This is the story of how light-mem evolved from a simple idea to a production-ready system, and the key architectural decisions that made it work.
...
const ThemeProvider = ({ children }) => {
  const [theme, setTheme] = useState<'light' | 'dark' | 'system'>(() => {
    return localStorage.getItem('light-mem-theme') || 'system';
  });

  useEffect(() => {
    localStorage.setItem('light-mem-theme', theme);
  }, [theme]);
...
**Impact**: Cross-platform compatibility restored, Windows users can now use light-mem without issues.
...
```bash
cp ~/.light-mem/light-mem.db ~/.light-mem/light-mem-v3-backup.db
```
