...
When `LIGHT_MEM_AUTH_MODE=api-key`, send `Authorization: Bearer <key>`. Read endpoints require `memories:read`; write endpoints require `memories:write`.
...
(`light-mem server worker start`); the HTTP path never blocks on a
provider response.
