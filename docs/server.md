# Light-Mem Server (Beta)

Light-Mem Server is the beta server runtime for Light-Mem 13. It is a
Postgres-backed, BullMQ-driven, API-key-authenticated runtime that replaces
the legacy `light-mem worker` for deployable use cases.
...
2. Generation can scale horizontally (`docker compose up --scale light-mem-worker=N`).
...
|-----------------------------------|----------|--------------------------------------------------------------|
| `LIGHT_MEM_RUNTIME`              | Docker   | Must be `server-beta` in Docker (warned otherwise).          |
| `LIGHT_MEM_QUEUE_ENGINE`         | Docker   | Must be `bullmq`. In-process queues are rejected in Docker.  |
| `LIGHT_MEM_SERVER_DATABASE_URL`  | Always   | Postgres connection string. Fails fast at startup.           |
| `LIGHT_MEM_REDIS_URL`            | bullmq   | Required when queue engine is `bullmq`.                      |
| `LIGHT_MEM_AUTH_MODE`            | Always   | Must NOT be `local-dev` in Docker.                           |
| `LIGHT_MEM_ALLOW_LOCAL_DEV_BYPASS` | Docker | Must NOT be `1`/`true` in Docker.                            |
| `LIGHT_MEM_GENERATION_DISABLED`  | Optional | Set to `true` on the HTTP service when running a separate worker. |
| `LIGHT_MEM_SERVER_PROVIDER`      | Worker   | One of `claude`, `gemini`, `openrouter`. Worker only.        |
...
## Generation worker mode (`light-mem server worker start`)

The same image runs the generation worker via:

```sh
light-mem server worker start
```
...
* Forces generation enabled even if `LIGHT_MEM_GENERATION_DISABLED=true`
  is inherited from the shared compose file. The worker IS the generation
  process.

In Compose this is the `light-mem-worker` service. Scale it horizontally:

```sh
docker compose up -d --scale light-mem-worker=4
```
...
```sh
light-mem server api-key create \
  --name "ci"                  \
...
```sh
light-mem server api-key revoke <id>
```
...
> **Do not enable `LIGHT_MEM_AUTH_MODE=local-dev` in Docker.** The
...
* `light-mem-server` — HTTP runtime.
  `LIGHT_MEM_GENERATION_DISABLED=true` so the BullMQ Worker is **not**
  attached here.
* `light-mem-worker` — generation worker. Scale horizontally.
...
* Restart of `light-mem-server` and `light-mem-worker` mid-stream does
  not lose data.
* Revoking an API key denies subsequent reads and writes (401/403).
* No `worker-service.cjs` process runs in any container.
* `LIGHT_MEM_AUTH_MODE=local-dev` is rejected inside Docker.
