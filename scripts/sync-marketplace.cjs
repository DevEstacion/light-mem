...
const INSTALLED_PATH = path.join(os.homedir(), '.claude', 'plugins', 'marketplaces', 'light-mem');
const CACHE_BASE_PATH = path.join(os.homedir(), '.claude', 'plugins', 'cache', 'light-mem', 'light-mem');
...
try {
  ...
  execSync(
    `rsync -av --delete --exclude=.git --exclude=bun.lock --exclude=package-lock.json --exclude=scripts/package.json --exclude=scripts/node_modules ${gitignoreExcludes} ./ ~/.claude/plugins/marketplaces/light-mem/`,
    { stdio: 'inherit' }
  );

  console.log('Running npm install in marketplace...');
  execSync(
    'cd ~/.claude/plugins/marketplaces/light-mem/ && npm install --omit=dev --no-audit --no-fund',
    { stdio: 'inherit' }
  );
  ...
  execSync(`npm install --omit=dev --no-audit --no-fund`, { cwd: CACHE_VERSION_PATH, stdio: 'inherit' });
