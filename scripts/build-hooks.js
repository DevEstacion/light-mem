...
async function verifyShellTemplateCanonical() {
  ...
  // Rule C safety net (node-runner.js fixBrokenScriptPath) must stay documented.
  const nodeRunner = fs.readFileSync('plugin/scripts/node-runner.js', 'utf-8');
  if (!nodeRunner.includes('function fixBrokenScriptPath')) {
    throw new Error(
      'plugin/scripts/node-runner.js is missing fixBrokenScriptPath — it is the Rule C runtime safety net behind Rule A. Do not remove it.'
    );
  }

  // Parser-compat guard (issue #2791): node-runner.js is invoked by hosts that
  // may run a pre-ES2020 Node whose ESM loader throws on optional chaining.
  // Strip comments, then forbid `?.` / `??` in executable code.
  const nodeRunnerCode = nodeRunner
    .replace(/\/\*[\s\S]*?\*\//g, '')
    .replace(/(^|[^:])\/\/.*$/gm, '$1');
  if (/\?\.|\?\?/.test(nodeRunnerCode)) {
    throw new Error(
      'plugin/scripts/node-runner.js uses optional chaining (?.) or nullish coalescing (??) — ' +
...
async function buildHooks() {
  console.log('🔨 Building light-mem hooks and worker service...\n');
  ...
  try {
    ...
    const pluginPackageJson = {
      name: 'light-mem-plugin',
      version: version,
      private: true,
      description: 'Runtime dependencies for light-mem bundled hooks',
      type: 'module',
      dependencies: {
        'zod': '^4.4.3',
    ...
    await build({
      ...
      minify: true,
      logLevel: 'error',
      // Externalize zod for consistency with worker-service —
    ...
    if (fs.existsSync('src/integrations/opencode-plugin/index.ts')) {
      ...
      // Also emit into the plugin/ tree, which is what sync-marketplace rsyncs to
      // the installed MARKETPLACE_ROOT. OpenCodeInstaller probes
    ...
    console.log('\n📋 Copying potion-base-8m model to plugin tree...');
    const modelSrcDir = 'src/models/potion-base-8m';
    const modelDstDir = 'plugin/models/potion-base-8m';
    const modelFiles = ['model.safetensors', 'tokenizer.json'];
    fs.mkdirSync(modelDstDir, { recursive: true });
    for (const modelFile of modelFiles) {
      const src = path.join(modelSrcDir, modelFile);
      const dst = path.join(modelDstDir, modelFile);
