...
async function getDatabaseInfo(
  ...
  try {
    const dbPath = path.join(dataDir, "light-mem.db");
...
export async function collectDiagnostics(
  ...
  const homeDir = os.homedir();
  const dataDir = path.join(homeDir, ".light-mem");
  ...
  const isDevMode = cwd.includes("light-mem") && !cwd.includes(".claude");

  const [nordMem, claudeCode, bun, osVersion] = await Promise.all([
    getClaudememVersion(),
  ...
  const database = {
    path: sanitizePath(path.join(dataDir, "light-mem.db")),
...
export function formatDiagnostics(diagnostics: SystemDiagnostics): string {
  ...
  output += "## Environment\n\n";
  output += `- **light-mem**: ${diagnostics.versions.nordMem}\n`;
