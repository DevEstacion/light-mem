...
function resolveWorkerPort(): number {
  const raw = process.env.LIGHT_MEM_WORKER_PORT;
  if (raw === undefined || raw === '') return DEFAULT_WORKER_PORT;
  const parsed = parseInt(raw, 10);
  if (!Number.isInteger(parsed) || parsed < 1 || parsed > 65535) {
    console.warn(
      `[check-pending-queue] Invalid LIGHT_MEM_WORKER_PORT=${JSON.stringify(raw)}; ` +
...
async function main() {
  ...
  if (args.includes('--help') || args.includes('-h')) {
    console.log(`
Light-Mem Pending Queue Manager
...
Environment:
  LIGHT_MEM_WORKER_PORT  Worker port (default: 37777)
...
What is this for?
  If the light-mem worker has unprocessed observations queued, this script
...
`);
  ...
  console.log('\n=== Light-Mem Pending Queue Status ===\n');

  const healthy = await checkWorkerHealth();
  if (!healthy) {
    console.log(`Worker is not running at ${WORKER_URL}. Start it with:`);
    console.log('  cd ~/.claude/plugins/marketplaces/light-mem && npm run worker:start\n');
    process.exit(1);
  }
