...
function checkPluginClosure(failures) {
  ...
  // Runtime install parity with version-check.js ensurePluginDependencies —
  // `npm install --omit=dev`. This reproduces the dependency tree a user gets
  // when the plugin's runtime deps are materialized, so a broken closure (e.g.
  // an unresolvable zod subpath) surfaces here.
  log('  Running: npm install --omit=dev --ignore-scripts');
  try {
    ...
    failures.push(`npm install failed in fresh plugin temp dir: ${out || error.message}`);
    return;
  }
  ...
  const res = spawnSync('node', [workerEntry, '--version'], {
    cwd: tmpPlugin,
    encoding: 'utf8',
    timeout: 20000,
...
function checkPackageCompleteness(failures) {
  ...
  const pkgRoot = path.join(tmpPkg, 'node_modules', 'light-mem');
  ...
  // (the real, runnable user entry — `npx light-mem`), which exposes a safe
