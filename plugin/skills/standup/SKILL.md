...
The room is one shared file (default `~/.light-mem/STANDUP.md`): YAML front
...
> `~/.light-mem/STANDUP.md`; speak with `node "<skill-dir>/standup.mjs" post …`,
...
```
Defaults: agent = git branch, file = `~/.light-mem/STANDUP.md`. Every write is
atomically locked.
