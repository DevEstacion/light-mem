...
description: Automated semantic versioning and release workflow for Claude Code plugins. Handles version increments across package.json, marketplace.json, plugin.json manifests, npm publishing (so `npx light-mem@X.Y.Z` resolves), build verification, git tagging, GitHub releases, and changelog generation.
---
...
3.  **Paths — every file that carries the version string**:
    - `package.json` — **the npm/npx-published version** (`npx light-mem@X.Y.Z` resolves from this)
...
7.  **Publish to npm** (this is what makes `npx light-mem@X.Y.Z` work):
    ```bash
    npm publish
    ```
    ...
    ```bash
    npm view light-mem@X.Y.Z version   # should print X.Y.Z
    ```
...
11. **Notify**: Run the Discord notification from `~/Scripts/light-mem/`, where the `.env` with Discord webhook details lives:
    ```bash
    cd ~/Scripts/light-mem/ && npm run discord:notify vX.Y.Z
    ```
...
- [ ] **`npm publish` succeeded and `npm view light-mem@X.Y.Z version` confirms it** (so `npx light-mem@X.Y.Z` resolves)
- [ ] GitHub release created with notes
- [ ] `CHANGELOG.md` updated and pushed
- [ ] Discord notification run from `~/Scripts/light-mem/`
