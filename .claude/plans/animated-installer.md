# Comprehensive Light-Mem Installer with @clack/prompts

## Overview

Build a beautiful, animated CLI installer for light-mem using `@clack/prompts` (v1.0.1). Distributable via `npx light-mem-installer` and `curl -fsSL https://install.cmem.ai | bash`. Replaces the need for users to manually clone, build, configure settings, and start the worker.
...
1. **Create directory structure** in the worktree:
   ...
   │       └── settings-writer.ts # Write ~/.light-mem/settings.json
...
2. **Create `package.json`**:
   ```json
   {
     "name": "light-mem-installer",
     "version": "1.0.0",
     "type": "module",
     "bin": { "light-mem-installer": "./dist/index.js" },
     "files": ["dist"],
     "scripts": {
...
1. **`src/index.ts`** — Entry point:
   - TTY guard: if `!process.stdin.isTTY`, print error directing user to `npx light-mem-installer`, exit 1
   - Import and call `runInstaller()` from steps
   - Top-level catch → `p.cancel()` + exit 1

2. **`src/steps/welcome.ts`** — Welcome step:
   - `p.intro()` with styled title using picocolors: `" light-mem installer "`
   - Display version info via `p.log.info()`
   - Check if already installed (detect `~/.light-mem/settings.json` and `~/.claude/plugins/marketplaces/light-mem/`)
   - If upgrade detected, `p.confirm()`: "light-mem is already installed. Upgrade?"
...
1. **`src/steps/settings.ts`** — Settings wizard:
   ...
   - If customizing, use `p.group()` for:
     ...
     - **Data directory**: `p.text()` with default `~/.light-mem`
...
2. **`src/utils/settings-writer.ts`** — Write settings:
   ...
   - Write to `~/.light-mem/settings.json`
   - Create `~/.light-mem/` directory if it doesn't exist
...
1. **`src/steps/install.ts`** — Installation runner:
   - Use `p.tasks()` for visual progress:
     - **"Cloning light-mem repository"**: `git clone --depth 1 https://github.com/Ronald-Estacion_NordTech/light-mem.git` to temp dir
     - **"Installing dependencies"**: `npm install` in cloned repo
     - **"Building plugin"**: `npm run build` in cloned repo
     - **"Registering plugin"**: Copy plugin files to `~/.claude/plugins/marketplaces/light-mem/`
...
2. **`src/steps/worker.ts`** — Worker startup:
   - Use `p.spinner()` for worker startup:
     ...
     - Write PID file to `~/.light-mem/worker.pid`
...
### Verification
- [ ] Plugin files exist at `~/.claude/plugins/marketplaces/light-mem/`
