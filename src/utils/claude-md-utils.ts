...
export function getTargetFilename(settings?: ReturnType<typeof SettingsDefaultsManager.loadFromFile>): string {
  ...
  return s.LIGHT_MEM_FOLDER_USE_LOCAL_MD === 'true' ? CLAUDE_LOCAL_MD_FILENAME : CLAUDE_MD_FILENAME;
}
...
export function replaceTaggedContent(existingContent: string, newContent: string): string {
  const startTag = '<light-mem-context>';
  const endTag = '</light-mem-context>';
...
export async function updateFolderClaudeMdFiles(
  ...
  const limit = parseInt(settings.LIGHT_MEM_CONTEXT_OBSERVATIONS, 10) || 50;
  const targetFilename = getTargetFilename(settings);

  let folderMdExcludePaths: string[] = [];
  try {
    const parsed = JSON.parse(settings.LIGHT_MEM_FOLDER_MD_EXCLUDE || '[]');
    if (Array.isArray(parsed)) {
      folderMdExcludePaths = parsed.filter((p): p is string => typeof p === 'string');
    }
  } catch {
    logger.warn('FOLDER_INDEX', 'Failed to parse LIGHT_MEM_FOLDER_MD_EXCLUDE setting');
  }

  // #2400 — deny-list of glob patterns where an empty/skeleton CLAUDE.md must
  // NOT be injected. Unlike LIGHT_MEM_FOLDER_MD_EXCLUDE (which excludes the
  ...
  try {
    const parsed = JSON.parse(settings.LIGHT_MEM_FOLDER_MD_SKELETON_DENYLIST || '[]');
    if (Array.isArray(parsed)) {
      skeletonDenylistPatterns = parsed.filter((p): p is string => typeof p === 'string');
    }
  } catch {
    logger.warn('FOLDER_INDEX', 'Failed to parse LIGHT_MEM_FOLDER_MD_SKELETON_DENYLIST setting');
  }
