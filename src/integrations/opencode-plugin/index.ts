 ...
 * object whose keys are OpenCode's real hook names. The hooks light-mem binds
 ...
 * `event.type`. The only bus event types light-mem reacts to are
...
async function workerGetText(path: string): Promise<string | null> {
  ...
  try {
    ...
    if (!response.ok) {
      console.warn(`[light-mem] Worker GET ${path} returned ${response.status}`);
      return null;
    }
...
export const LightMemPlugin = async (ctx: OpenCodePluginContext) => {
  const projectName = ctx.project?.name || "opencode";

  console.log(`[light-mem] OpenCode plugin loading (project: ${projectName})`);
  ...
  return {
    ...
    tool: {
      light_mem_search: {
        description:
          "Search light-mem memory database for past observations, sessions, and context",
        args: {
        ...
        async execute(args: Record<string, unknown>): Promise<string> {
          ...
          if (!text) {
            return "light-mem worker is not running. Start it with: npx light-mem start";
          }
...
export function parseSearchResponse(text: string, query: string): string {
  ...
  try {
    ...
    console.warn(
      "[light-mem] Failed to parse search results:",
