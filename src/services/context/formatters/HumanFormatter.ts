...
export function renderHumanContextIndex(): string[] {
  return [
    ...
    `${colors.dim}  - Fetch by ID: search(mode="fetch", ids=[IDs]) for observations visible in this index${colors.reset}`,
    `${colors.dim}  - Search history: Use the smart-search skill for past decisions, bugs, and deeper research${colors.reset}`,
...
export function renderHumanFooter(totalDiscoveryTokens: number, totalReadTokens: number): string[] {
  ...
  return [
    '',
    `${colors.dim}Access ${workTokensK}k tokens of past research & decisions for just ${totalReadTokens.toLocaleString()}t. Use the light-mem skill to access memories by ID.${colors.reset}`
  ];
}
