...
const PLACEHOLDER_CONTEXT = `# light-mem: Cross-Session Memory

*No context yet. Complete your first session and context will appear here.*

Use light-mem's MCP search tools for manual memory queries.`;
...
function writeMcpJsonConfig(
  ...
  existingConfig[serversKeyName]['light-mem'] = buildMcpServerEntry(mcpServerPath);
...
function installMcpIntegration(config: McpInstallerConfig): () => Promise<number> {
  return async (): Promise<number> => {
    console.log(`\nInstalling Light-Mem MCP integration for ${config.ideLabel}...\n`);

    const mcpServerPath = getMcpServerAbsolutePath();
    if (!mcpServerPath) {
      console.error('Could not find MCP server script');
      console.error('   Expected at: ~/.claude/plugins/marketplaces/light-mem/plugin/scripts/mcp-server.cjs');
      return 1;
    }
...
function writeMcpConfigAndContext(
  ...
  summaryLines.push('  1. Start light-mem worker: npx light-mem start');
...
const ANTIGRAVITY_CONFIG: McpInstallerConfig = {
  ...
  contextFile: {
    path: path.join(process.cwd(), '.agents', 'rules', 'light-mem-context.md'),
    isWorkspaceRelative: true,
  },
};
...
const ROO_CODE_CONFIG: McpInstallerConfig = {
  ...
  configKey: 'mcpServers',
  contextFile: {
    path: path.join(process.cwd(), '.roo', 'rules', 'light-mem-context.md'),
    isWorkspaceRelative: true,
  },
};
...
function gooseConfigHasLightMemEntry(yamlContent: string): boolean {
  return yamlContent.includes('light-mem:') &&
...
function buildGooseLightMemEntryYaml(mcpServerPath: string): string {
  return [
    '  light-mem:',
    `    command: ${getNodeAbsolutePath()}`,
    '    args:',
...
export async function installGooseMcpIntegration(): Promise<number> {
  console.log('\nInstalling Light-Mem MCP integration for Goose...\n');
...
function mergeGooseYamlConfig(configPath: string, mcpServerPath: string): void {
  if (existsSync(configPath)) {
    ...
    if (gooseConfigHasLightMemEntry(yamlContent)) {
      const nordMemPattern = /( {2}light-mem:\n(?:.*\n)*?(?= {2}\S|\n\n|^\S|$))/m;
      const newEntry = buildGooseLightMemEntryYaml(mcpServerPath) + '\n';

      if (!nordMemPattern.test(yamlContent)) {
        throw new Error('Found mcpServers/light-mem markers but could not locate a replaceable light-mem block');
      }
      yamlContent = yamlContent.replace(nordMemPattern, newEntry);
      writeFileSync(configPath, yamlContent);
      console.log(`  Updated existing light-mem entry in: ${configPath}`);
      ...
      const newEntry = '\n' + buildGooseLightMemEntryYaml(mcpServerPath);
      ...
      writeFileSync(configPath, yamlContent);
      console.log(`  Added light-mem to existing mcpServers in: ${configPath}`);
    } else {
