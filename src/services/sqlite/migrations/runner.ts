...
export class MigrationRunner {
  ...
  runAllMigrations(): void {
    ...
    this.createVectorsTable();
  }

  /**
   * Migration 35: local vector store. Replaces the external Chroma collection
   * with an in-process table holding one row per embedded document (the same
   * 1-to-many doc-id scheme ChromaSync used: obs_{id}_narrative,
   * obs_{id}_fact_0, summary_{id}_request, prompt_{id}, ...). `embedding` is a
   * raw little-endian Float32 BLOB (256 floats = 1024 bytes) produced by the
   * bundled potion-base-8M embedder. `document` is kept so queries can fuse
   * cosine similarity with BM25 over the same text.
   */
  private createVectorsTable(): void {
    ...
    `);
    this.db.run('CREATE INDEX IF NOT EXISTS idx_vectors_collection ON vectors(collection)');
    this.db.run('CREATE INDEX IF NOT EXISTS idx_vectors_project ON vectors(collection, project)');
    this.db.run('CREATE INDEX IF NOT EXISTS idx_vectors_sqlite ON vectors(collection, doc_type, sqlite_id)');

    this.db.prepare('INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)').run(35, new Date().toISOString());
    logger.debug('DB', 'Created vectors table for local potion-base-8M store (migration 35)');
  }
