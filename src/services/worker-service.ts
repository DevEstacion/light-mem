...
export class WorkerService implements WorkerRef {
  ...
  private registerRoutes(): void {
    ...
    this.server.app.use(['/api', '/v1'], async (req, res, next) => {
      if (req.path === '/health' || req.path === '/readiness' || req.path === '/version') {
        next();
        return;
      }
    ...
    const sessionRoutes = new SessionRoutes(this.sessionManager, this.dbManager, this.sdkAgent, this.sessionEventBroadcaster, this, this.completionHandler);
    ...
    this.server.registerRoutes(new MemoryRoutes(this.dbManager, 'light-mem'));
  ...
  private async initializeBackground(): Promise<void> {
    try {
      ...
      const modeId = settings.LIGHT_MEM_MODE;
      ModeManager.getInstance().loadMode(modeId);
      logger.info('SYSTEM', `Mode loaded: ${modeId}`);

      if (settings.LIGHT_MEM_MODE === 'local' || !settings.LIGHT_MEM_MODE) {
      ...
      if (this.dbManager.getChromaSync()) {
  ...
  private async startTranscriptWatcher(settings: ReturnType<typeof SettingsDefaultsManager.loadFromFile>): Promise<void> {
    const transcriptsEnabled = settings.LIGHT_MEM_TRANSCRIPTS_ENABLED !== 'false';
    if (!transcriptsEnabled) {
      logger.info('TRANSCRIPT', 'Transcript watcher disabled via LIGHT_MEM_TRANSCRIPTS_ENABLED=false');
      return;
    }

    const configPath = settings.LIGHT_MEM_TRANSCRIPTS_CONFIG_PATH || DEFAULT_CONFIG_PATH;
...
async function main() {
  ...
  switch (command) {
    ...
    case 'hook': {
      ...
      if (!platform || !event) {
        console.error('Usage: light-mem hook <platform> <event>');
    ...
    case 'transcript': {
      ...
      // (see light-mem/light-mem 2450). Dispatch to the shared
...
// Entrypoint detection. The bundled worker runs as `node worker-service.cjs`,
// where `require.main === module` is the authoritative signal. The previous
// bare `!module.parent` fallback is too loose under test runners (Vitest
// imports this module with a null `module.parent`, which would falsely fire
// main() and call process.exit) — so the parent-less fallback now also requires
// argv[1] to actually point at the worker script.
const argv1 = process.argv[1]?.replaceAll('\\', '/') ?? '';
const argvIsWorker = argv1.endsWith('worker-service')
  || argv1.endsWith('worker-service.cjs')
  || (typeof __filename !== 'undefined' && argv1 === __filename.replaceAll('\\', '/'));
const isMainModule = typeof require !== 'undefined' && typeof module !== 'undefined'
  ? require.main === module
    || process.env.LIGHT_MEM_MANAGED === 'true'
    || (!module.parent && argvIsWorker)
  : import.meta.url === `file://${process.argv[1]}` || argvIsWorker;
