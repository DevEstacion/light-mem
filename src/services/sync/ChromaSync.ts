...
export class ChromaSync {
  ...
  private async ensureCollectionExists(): Promise<void> {
    ...
    // Collections are virtual in the local store (a `collection` column on the
    // vectors table); this is a no-op kept for call-site symmetry.
    LocalVectorStore.getInstance().createCollection(this.collectionName);
  ...
  private async addDocuments(documents: ChromaDocument[]): Promise<number> {
    ...
    for (let i = 0; i < documents.length; i += this.BATCH_SIZE) {
      ...
      try {
        // Upsert is native (ON CONFLICT) in the local store, so the old
        // delete+add reconcile path Chroma needed is gone.
        store.addDocuments(this.collectionName, cleanBatch);
  ...
  private async getExistingChromaIds(projectOverride?: string): Promise<{
    ...
    logger.info('CHROMA_SYNC', 'Fetching existing vector document IDs...', { project: targetProject });

    while (true) {
      const result = store.getDocuments(this.collectionName, {
  ...
  async ensureBackfilled(projectOverride?: string, storeOverride?: SessionStore): Promise<void> {
    ...
    // One-time migration self-heal: a watermark file left over from the old
    // Chroma store claims everything is synced, but the local `vectors` table
    // starts empty. If THIS PROJECT has zero vectors yet a watermark says
    // otherwise, reset the watermark so its corpus re-embeds locally. Scoped
    // per-project because the collection (cm__light-mem) is shared — a total
    // count would be masked by other already-migrated projects, leaving a
    // SIGKILL-interrupted project permanently un-embedded.
    if (LocalVectorStore.getInstance().countForProject(this.collectionName, backfillProject) === 0) {
      const wm = ChromaSyncState.get(backfillProject);
      if (wm.observations > 0 || wm.summaries > 0 || wm.prompts > 0) {
        logger.info('CHROMA_SYNC', 'Local vector store empty but watermark non-zero — resetting to re-embed (Chroma→local migration)', {
          project: backfillProject,
          staleWatermark: wm
        });
        ChromaSyncState.replace(backfillProject, { observations: 0, summaries: 0, prompts: 0 });
      }
    }
  ...
  async queryChroma(
    ...
    try {
      const store = LocalVectorStore.getInstance();
      results = store.query(this.collectionName, query, limit, whereFilter);
    } catch (error) {
   ...
   * Uses a single shared ChromaSync('light-mem') instance and Chroma connection.
   * Per-project scoping is passed as a parameter to ensureBackfilled(), avoiding
   * instance state mutation. All documents land in the cm__light-mem collection
  ...
  async updateMergedIntoProject(
    ...
    for (let i = 0; i < sqliteIds.length; i += this.BATCH_SIZE) {
      ...
      const existing = store.getDocuments(this.collectionName, {
      ...
      store.updateDocuments(this.collectionName, docIds, metadatas);
