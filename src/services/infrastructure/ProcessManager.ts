...
function resolveWorkerRuntimePathUncached(options: RuntimeResolverOptions): string | null {
  // The worker runs under Node now (node:sqlite via the compat adapter), not
  // Bun. Hooks are invoked by Claude Code through `node .../node-runner.js`, so
  // `process.execPath` is the Node binary that launched us — the correct
  // runtime to spawn the worker daemon with. No Bun discovery needed.
...
export function runOneTimeCwdRemap(dataDirectory?: string): void {
  ...
  const dbPath = path.join(effectiveDataDir, 'light-mem.db');
...
export function spawnDaemon(
  ...
  const env = sanitizeEnv({
    ...process.env,
    LIGHT_MEM_WORKER_PORT: String(port),
    ...extraEnv
  });
  ...
  if (!runtimePath) {
    logger.error(
      'SYSTEM',
      'Node runtime path could not be resolved for the worker daemon (process.execPath was empty).'
    );
    return undefined;
  }
