 ...
 * ~/.light-mem/.env, see #2357) is environmental — re-logging it on every
...
export function classifyClaudeError(err: unknown): ClassifiedProviderError {
  ...
  // HTTP 400 from the Anthropic SDK — bad request, never recoverable.
    ...
    if (/effort parameter/i.test(haystack) && !effortHintLogged) {
      ...
      logger.warn(
        ...
        'Anthropic API rejected request with HTTP 400: this model does not support the `effort` parameter. ' +
          'CLAUDE_CODE_EFFORT_LEVEL is likely leaking into the SDK subprocess env via ~/.light-mem/.env — ' +
          'remove it or scope it to models that support effort. See https://github.com/Ronald-Estacion_NordTech/light-mem/issues/2357.',
        { status: 400 }
      );
    }
  ...
  async startSession(session: ActiveSession, worker?: WorkerRef): Promise<void> {
    ...
    const queryResult = query({
      ...
      options: buildHardenedSdkOptions({
        ...
        model: modelId,
        env: isolatedEnv,  // Use isolated credentials from ~/.light-mem/.env, not process.env
      ...
      for await (const message of queryResult) {
        ...
        if (message.type === 'assistant') {
          ...
          if (typeof textContent === 'string' && textContent.includes('Invalid API key')) {
            throw new Error('Invalid API key: check your API key configuration in ~/.light-mem/settings.json or ~/.light-mem/.env');
          }
