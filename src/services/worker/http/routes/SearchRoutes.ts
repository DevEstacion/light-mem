...
// enough that toggling LIGHT_MEM_WELCOME_HINT_ENABLED is responsive in
// practice and long enough to absorb hook bursts.
const SETTINGS_CACHE_TTL_MS = 5000;

const WELCOME_HINT_TEMPLATE = `# light-mem status
...
export class SearchRoutes extends BaseRouteHandler {
  ...
  private handleContextInject = this.wrapHandler(async (req: Request, res: Response): Promise<void> => {
    ...
    // settings cache TTL.
    const hintEnabledRaw = process.env.LIGHT_MEM_WELCOME_HINT_ENABLED ?? settings.LIGHT_MEM_WELCOME_HINT_ENABLED;
    ...
    if (hintEnabled && !full) {
      ...
      if (!this.projectsHaveObservations(sessionStore, projects)) {
        const port = process.env.LIGHT_MEM_WORKER_PORT ?? settings.LIGHT_MEM_WORKER_PORT;
  ...
  private handleSearchHelp = this.wrapHandler((req: Request, res: Response): void => {
    ...
    res.json({
      ...
      examples: [
        ...
        `curl "${baseUrl}/api/context/recent?project=light-mem&limit=3"`,
