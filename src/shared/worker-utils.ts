...
function getWorkerSettingsPath(): string {
  return path.join(SettingsDefaultsManager.get('LIGHT_MEM_DATA_DIR'), 'settings.json');
}
...
export function getWorkerPort(): number {
  ...
  cachedPort = parseInt(settings.LIGHT_MEM_WORKER_PORT, 10);
  return cachedPort;
}
...
export function getWorkerHost(): string {
  ...
  const settings = getWorkerSettings();
  cachedHost = settings.LIGHT_MEM_WORKER_HOST;
  return cachedHost;
}
...
function getFailLoudThreshold(): number {
  try {
    ...
    const raw = settings.LIGHT_MEM_HOOK_FAIL_LOUD_THRESHOLD;
...
export async function recordWorkerUnreachable(): Promise<number> {
  ...
  if (next.consecutiveFailures >= threshold) {
    ...
    emitBlockingError(
      `light-mem worker unreachable for ${next.consecutiveFailures} consecutive hooks.`
    );
  }
...
const WORKER_FALLBACK_BRAND: unique symbol = Symbol.for('light-mem/worker-fallback');
