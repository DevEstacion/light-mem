...
export function resolveDataDir(): string {
  if (process.env.LIGHT_MEM_DATA_DIR) {
    return process.env.LIGHT_MEM_DATA_DIR;
  }

  const defaultDataDir = join(homedir(), '.light-mem');
  const settingsPath = join(defaultDataDir, 'settings.json');
  try {
    if (existsSync(settingsPath)) {
      const raw = JSON.parse(readFileSync(settingsPath, 'utf-8'));
      const settings = raw.env ?? raw;
      if (settings.LIGHT_MEM_DATA_DIR) {
        return settings.LIGHT_MEM_DATA_DIR;
      }
    }
  } catch {
...
export const MARKETPLACE_ROOT = join(CLAUDE_CONFIG_DIR, 'plugins', 'marketplaces', 'light-mem');
...
export const DB_PATH = join(DATA_DIR, 'light-mem.db');
...
export const paths = {
  ...
  database: () => join(DATA_DIR, 'light-mem.db'),
