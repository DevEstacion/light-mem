...
// temp file via LIGHT_MEM_ENV_FILE without depending on module-load order.
// Production callers see the canonical ~/.light-mem/.env path through
// paths.envFile() unchanged.
export function envFilePath(): string {
  return process.env.LIGHT_MEM_ENV_FILE ?? paths.envFile();
}
...
const BLOCKED_ENV_VARS = [
  ...
  'ANTHROPIC_AUTH_TOKEN',    // Same leak risk as ANTHROPIC_API_KEY; a token inherited from the
                             ...
                             // The fresh token from ~/.light-mem/.env is re-injected below
  ...
  'ANTHROPIC_BASE_URL',      // Issue #2375: same leak class as AUTH_TOKEN. A leaked BASE_URL
                             ...
                             // Re-injected from ~/.light-mem/.env when configured.
...
function serializeEnvFile(env: Record<string, string>): string {
  const lines: string[] = [
    '# light-mem credentials',
    '# This file stores keys and gateway settings for the light-mem memory agent',
    '# Edit this file or use light-mem settings to configure',
    '',
  ];
 ...
 * are populated ONLY from ~/.light-mem/.env — never from the parent shell.
 ...
 */
export function loadLightMemEnv(): LightMemEnv {
...
export function saveLightMemEnv(env: LightMemEnv): void {
  const envFile = envFilePath();
...
export function buildIsolatedEnv(includeCredentials: boolean = true): Record<string, string> {
  ...
  isolatedEnv.LIGHT_MEM_INTERNAL = '1';

  if (includeCredentials) {
    const credentials = loadLightMemEnv();
...
export async function buildIsolatedEnvWithFreshOAuth(
  ...
  // gateway-appropriate token in ~/.light-mem/.env if their gateway requires
  ...
  // ~/.light-mem/.env (see loadLightMemEnv re-injection above). A BASE_URL
...
export function getCredential(key: keyof LightMemEnv): string | undefined {
...
export function getAuthMethodDescription(): string {
  if (hasAnthropicApiKey()) {
    return 'API key (from ~/.light-mem/.env)';
  }
  if (hasAnthropicAuthToken()) {
    return 'Gateway auth token (from ~/.light-mem/.env)';
  }
