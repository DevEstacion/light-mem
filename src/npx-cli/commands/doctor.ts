/**
 * `npx light-mem doctor` — a minimal diagnostic that probes every layer an
...
export async function runDoctorCommand(): Promise<void> {
  ...
  // 1. Node (required — the worker runs on Node ≥24 for built-in node:sqlite).
  const nodeVersion = probeVersion('node');
  const nodeMajor = nodeVersion ? parseInt(nodeVersion.replace(/^v/, ''), 10) : 0;
  ...
  checks.push({
    ...
    detail: nodeOk
      ...
      : nodeVersion
        ? `v${nodeVersion.replace(/^v/, '')} — light-mem needs Node ≥24 (built-in node:sqlite). Upgrade: nvm install 24`
        : 'not found on PATH — install Node ≥24: https://nodejs.org',
    required: true,
  });
  ...
  checks.push({
    ...
    detail: installed ? marketplaceDirectory() : 'run `npx light-mem install`',
    required: true,
  });
  ...
  checks.push({
    ...
    detail: depsPresent ? 'node_modules present' : 'missing — run `npx light-mem repair`',
    required: installed,
  });
  ...
  let workerDetail = `no response on port ${workerPort} — start with \`npx light-mem start\``;
  try {
