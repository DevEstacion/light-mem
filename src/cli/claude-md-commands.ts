...
import { Database } from '../services/sqlite/node-sqlite-compat.js';
...
function formatObservationsForClaudeMd(observations: ObservationRow[], folderPath: string): string {
  ...
  lines.push('');
  lines.push('<!-- This section is auto-generated by light-mem. Edit content outside the tags. -->');
  lines.push('');
...
export async function generateClaudeMd(dryRun: boolean): Promise<number> {
  ...
  const observationLimit = parseInt(settings.LIGHT_MEM_CONTEXT_OBSERVATIONS, 10) || 50;
...
function cleanSingleFile(file: string, relativePath: string, dryRun: boolean): 'deleted' | 'cleaned' {
  ...
  const stripped = content.replace(/<light-mem-context>[\s\S]*?<\/light-mem-context>/g, '').trim();
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          try {
            ...
            if (content.includes('<light-mem-context>')) {
